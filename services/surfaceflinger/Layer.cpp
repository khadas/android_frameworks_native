...
const sp<GraphicBuffer> & rgaCopyBit(sp<GraphicBuffer> src_buf, const Rect& rect, int dst_width, int dst_height)
{
    ...
    if (src_handle == 0 || dst_handle == 0) {
        ...
        src = wrapbuffer_handle(src_handle, src_buf->getWidth(),
                                            src_buf->getHeight(),
                                            src_buf->getPixelFormat());
        dst = wrapbuffer_handle(dst_handle, dstBufferRGA[yuvIndex]->getWidth(),
                                            dstBufferRGA[yuvIndex]->getHeight(),
                                            dstBufferRGA[yuvIndex]->getPixelFormat());
...
#if RK_NV12_10_TO_P010_BY_NEON
    memcpy_to_p010(src_vaddr, dst_vaddr, (int)dstWidth, (int)dstHeight, (int)srcBuffer->getWidth());
#endif

#if RK_NV12_10_TO_NV12_BY_NEON
    memcpy_to_NV12(src_vaddr, dst_vaddr, (int)dstWidth, (int)dstHeight, (int)srcBuffer->getWidth());
#endif
...
void Layer::updateSnapshot(bool updateGeometry) {
    ...
    if (mBufferInfo.mBuffer != nullptr) {
        if (mBufferInfo.mBuffer->getPixelFormat() == HAL_PIXEL_FORMAT_YCrCb_NV12_10) {
            sp<GraphicBuffer> dstGraphicBuffer = nullptr;
#if RK_NV12_10_TO_NV12_BY_RGA
            dstGraphicBuffer = rgaCopyBit(mBufferInfo.mBuffer->getBuffer(), mBufferInfo.mCrop,
                                    mBufferInfo.mCrop.getWidth(), mBufferInfo.mCrop.getHeight());
#else
            dstGraphicBuffer = compatible_rk_nv12_10_format(mBufferInfo.mBuffer->getBuffer(),
                                    mBufferInfo.mCrop.getWidth(), mBufferInfo.mCrop.getHeight());
#endif
            std::shared_ptr<renderengine::ExternalTexture> mmBuffer = nullptr;
            mmBuffer = std::make_shared<renderengine::impl::ExternalTexture>(dstGraphicBuffer,
                                        mFlinger->getCompositionEngine().getRenderEngine(),
                                        renderengine::impl::ExternalTexture::Usage::READABLE);

            snapshot->externalTexture = mmBuffer;
        } else {
